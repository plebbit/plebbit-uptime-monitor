  ...
  for (const subplebbit of monitorState.subplebbitsMonitoring) {
    ...
    ;(async () => {
      try {
        ...
        const ipnsPubsubTopicRoutingCid = await pubsubTopicToDhtKey(ipnsNameToIpnsOverPubsubTopic(getPlebbitAddressFromPublicKey(subplebbitUpdate.signature.publicKey)))
        monitorState.subplebbits[subplebbit.address] = {...monitorState.subplebbits[subplebbit.address], ipnsPubsubTopic, ipnsPubsubTopicRoutingCid}
