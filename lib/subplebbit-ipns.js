        ...
        if (subplebbitUpdate.statsCid) {
          plebbit.fetchCid(subplebbitUpdate.statsCid)
            .then(res => {
              ...
              console.log(`fetched subplebbit stats for '${subplebbit.address}' ${subplebbitStats.allPostCount} posts`)
            ...
            .catch(e => {
              console.log(`failed to fetch subplebbit stats for '${subplebbit.address}': ${e.message}`)
            })
        }

        // the dht and http router key used to announce/find providers for ipns over pubsub for the subplebbit
        const ipnsPeersCid = await pubsubTopicToDhtKey(ipnsNameToIpnsOverPubsubTopic(getPlebbitAddressFromPublicKey(subplebbitUpdate.signature.publicKey)))

        fetchDhtPeers(ipnsPeersCid)
          .then(ipnsDhtPeers => {
            console.log(`fetched subplebbit ipns dht peers for '${subplebbit.address}' ${ipnsDhtPeers.length} peers`)
          ...
          .catch(e => {
            console.log(`failed to fetch subplebbit ipns dht peers for '${subplebbit.address}': ${e.message}`)
            prometheusObserveSubplebbitIpnsPeers(subplebbit.address, 'Dht')
          })

        fetchHttpRoutersPeers(ipnsPeersCid)
          .then(ipnsHttpRoutersPeers => {
            console.log(`fetched subplebbit ipns http routers peers for '${subplebbit.address}' ${ipnsHttpRoutersPeers.length} peers`)
          ...
          .catch(e => {
            console.log(`failed to fetch subplebbit ipns http routers peers for '${subplebbit.address}': ${e.message}`)
        ...
        if (subplebbitUpdate.updateCid) {
          fetchHttpRoutersPeers(subplebbitUpdate.updateCid)
            .then(ipnsCidHttpRoutersPeers => {
              console.log(`fetched subplebbit ipns cid http routers peers for '${subplebbit.address}' ${ipnsCidHttpRoutersPeers.length} peers`)
            ...
            .catch(e => {
              console.log(`failed to fetch subplebbit update cid http routers peers for '${subplebbit.address}': ${e.message}`)
