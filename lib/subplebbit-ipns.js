      ...
      try {
        ...
        if (subplebbitUpdate.statsCid) {
          try {
            ...
            monitorState.subplebbits[subplebbit?.address] = {...monitorState.subplebbits[subplebbit?.address], stats: subplebbitStats}
...
const gauges = {
  ...
  subplebbitStatsAllActiveUserCount: new prometheus.promClient.Gauge({
    name: `${prometheus.prefix}subplebbit_stats_all_active_user_count`,
    help: `gauge of subplebbit stats all active user count labeled with: ${gaugeLabelNames.join(', ')}`,
    labelNames: gaugeLabelNames, registers: [prometheus.promClient.register]
  }),
  subplebbitStatsAllPostCount: new prometheus.promClient.Gauge({
    name: `${prometheus.prefix}subplebbit_stats_all_post_count`,
    help: `gauge of subplebbit stats all post count labeled with: ${gaugeLabelNames.join(', ')}`,
    labelNames: gaugeLabelNames, registers: [prometheus.promClient.register]
  }),
  subplebbitStatsAllReplyCount: new prometheus.promClient.Gauge({
    name: `${prometheus.prefix}subplebbit_stats_all_reply_count`,
    help: `gauge of subplebbit stats all retry count labeled with: ${gaugeLabelNames.join(', ')}`,
    labelNames: gaugeLabelNames, registers: [prometheus.promClient.register]
  })
}
const histogramLabelNames = []
const histograms = {}
const prometheusObserveSubplebbitUpdate = (subplebbitAddress, subplebbitUpdate) => {
  const secondsSinceUpdatedAt = Math.ceil(Date.now() / 1000) - subplebbitUpdate.updatedAt || undefined
  // counters
  ...
  if (secondsSinceUpdatedAt && typeof secondsSinceUpdatedAt === 'number') {
...
const prometheusObserveSubplebbitStats = (subplebbitAddress, subplebbitStats) => {
  // gauges
  if (typeof subplebbitStats?.allActiveUserCount === 'number') {
    gauges.subplebbitStatsAllActiveUserCount.set({subplebbit_address: subplebbitAddress}, subplebbitStats?.allActiveUserCount)
  }
  if (typeof subplebbitStats?.allPostCount === 'number') {
    gauges.subplebbitStatsAllPostCount.set({subplebbit_address: subplebbitAddress}, subplebbitStats?.allPostCount)
  }
  if (typeof subplebbitStats?.allReplyCount === 'number') {
    gauges.subplebbitStatsAllReplyCount.set({subplebbit_address: subplebbitAddress}, subplebbitStats?.allReplyCount)
  }
}
