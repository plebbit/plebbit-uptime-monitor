...
const getPublishStats = async (pubsubProviderUrl) => {
  ...
  try {
    ...
    debug(`subscribing to pubsub topic '${pubsubTopic}' from '${config.ipfsApiUrl}'...`)
    ...
    kubo.pubsub.publish(pubsubTopic, Buffer.from(randomMessage2)) // don't await this, causes bug
      .catch(e => debug(`failed kubo.pubsub.publish from '${pubsubProviderUrl}': ${e.message}`))
    await waitForMessagePromise
  ...
  kubo.pubsub.unsubscribe(pubsubTopic)
    .catch(e => debug(`failed to pubsub unsubscribe from '${config.ipfsApiUrl}': ${e.message}`))
