...
import {kuboPubsub, kuboPubsubProviders, pubsubKuboApiUrl} from './plebbit-js/plebbit-js.js'
...
const getPublishStats = async (pubsubProviderUrl) => {
  ...
  try {
    ...
    debug(`subscribing to pubsub topic '${pubsubTopic}' from '${pubsubKuboApiUrl}'...`)
    ...
    debug(`subscribing to pubsub topic '${pubsubTopic}' (peers cid: ${pubsubTopicRoutingCid}) from pubsub provider '${pubsubProviderUrl}'...`)
  ...
  kuboPubsub.pubsub.unsubscribe(pubsubTopic)
    .catch(e => debug(`failed to pubsub unsubscribe from '${pubsubKuboApiUrl}': ${e.message}`))
