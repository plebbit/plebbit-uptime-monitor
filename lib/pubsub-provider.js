...
const getPublishStats = async (pubsubProviderUrl) => {
  ...
  const pubsubTopicPeersCid = await pubsubTopicToDhtKey(pubsubTopic)
  ...
  try {
    ...
    debug(`subscribing to pubsub topic '${pubsubTopic}' (peers cid: ${pubsubTopicPeersCid}) from pubsub provider '${pubsubProviderUrl}'...`)
