...
const debug = Debug('plebbit-uptime-monitor:webpage')

export const monitorWebpages = async () => {
  debug(`monitoring ${config.monitoring.webpages?.length} webpages: ${config.monitoring.webpages?.map(webpage => webpage.url).join(' ')}`)
