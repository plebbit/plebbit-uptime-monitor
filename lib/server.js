...
app.get('/', (req, res) => {
  ...
  const plebbit = {subplebbitsStats: {}, subplebbitCount: 0, ...getUniquePeerCounts()}
  const subplebbits = {}
  ...
  for (const pubsubProviderUrl in monitorState.pubsubProviders) {
  ...
  const httpRouters = {}
  for (const httpRouterUrl in monitorState.httpRouters) {
  ...
  for (const plebbitPreviewerUrl in monitorState.plebbitPreviewers) {
  ...
  for (const chainProviderUrl in monitorState.chainProviders) {
  ...
  const webpages = {}
  for (const webpageUrl in monitorState.webpages) {
    webpages[webpageUrl] = {
      url: webpageUrl,
  ...
  const nfts = {}
  for (const nftName in monitorState.nfts) {
    nfts[nftName] = {
      name: nftName,
...
const getUniquePeerCounts = () => {
  const peersMaps = {
    ipnsDhtPeers: new Map(), ipnsHttpRoutersPeers: new Map(), ipnsCidHttpRoutersPeers: new Map(),
    pubsubDhtPeers: new Map(), pubsubHttpRoutersPeers: new Map(), pubsubPeers: new Map()
  }
  for (const subplebbitAddress in monitorState.subplebbits) {
  ...
  const peerCounts = {}
  for (const peersName in peersMaps) {
    peerCounts[`${peersName}Count`] = peersMaps[peersName].size
  }
  return peerCounts
}
