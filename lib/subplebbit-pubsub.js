...
const monitorSubplebbitPubsub = async (subplebbit, pubsubTopic, subplebbitPublicKey) => {
  ...
  fetchDhtPeers(pubsubPeersCid)
    .then(pubsubDhtPeers => {
      console.log(`fetched subplebbit pubsub dht peers for '${subplebbit.address}' ${pubsubDhtPeers.length} peers`)
    ...
    .catch(e => {
      console.log(`failed to fetch subplebbit pubsub dht peers for '${subplebbit.address}': ${e.message}`)
  ...
  fetchHttpRoutersPeers(pubsubPeersCid)
    .then(pubsubHttpRoutersPeers => {
      console.log(`fetched subplebbit pubsub http routers peers for '${subplebbit.address}' ${pubsubHttpRoutersPeers.length} peers`)
    ...
    .catch(e => {
      console.log(`failed to fetch subplebbit pubsub http routers peers for '${subplebbit.address}': ${e.message}`)
  ...
  startListeningToPubsubMessages(subplebbit, pubsubTopic, subplebbitPublicKey)
    .catch(e => console.log(`failed start listening to pubsub message for '${subplebbit.address}': ${e.message}`))
...
const startListeningToPubsubMessages = async (subplebbit, pubsubTopic, subplebbitPublicKey) => {
  ...
  const onPubsubMessageReceived = (rawPubsubMessage) => {
    ...
    try {
      ...
      console.log(`got pubsub message in '${subplebbit?.address}'`)
      ...
      if (subplebbitPublicKey === pubsubMessagePublicKeyBase64) {
        ...
        console.log(`got pubsub message from subplebbit '${subplebbit?.address}'`)
      }
    }
    catch (e) {
  ...
  try {
    ...
    console.log(`subscribed to subplebbit pubsub '${subplebbit?.address}'`)
