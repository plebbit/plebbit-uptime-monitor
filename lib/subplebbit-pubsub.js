...
import {kuboPubsub, plebbitPubsubApi, pubsubApiUrl} from './plebbit-js/plebbit-js.js'
...
const monitorSubplebbitPubsub = async (subplebbit, pubsubTopic, subplebbitPublicKey) => {
  ...
  const pubsubRoutingCid = await pubsubTopicToDhtKey(pubsubTopic)
  monitorState.subplebbits[subplebbit.address] = {...monitorState.subplebbits[subplebbit.address], pubsubRoutingCid}

  fetchDhtPeers(pubsubRoutingCid)
    .then(pubsubDhtPeers => {
  ...
  fetchHttpRoutersPeers(pubsubRoutingCid)
...
const pubsubSubscribeRetryForever = (pubsubTopic, onMessage, subplebbitAddress) => {
  const onError = async (error, fatal, pubsubMessage) => {
    debug(`kubo.pubsub.subscribe onError '${pubsubApiUrl}' subplebbit '${subplebbitAddress}'`, {error, fatal, pubsubMessage})
    if (fatal) {
      try {
        await kuboPubsub.pubsub.unsubscribe(pubsubTopic, onMessage)
      }
      catch (e) {
        debug(`kubo.pubsub.unsubscribe error '${pubsubApiUrl}' subplebbit '${subplebbitAddress}': ${e.message}`)
      }
      await trySubscribe()
    }
  }
...
const publishFakeChallengeRequest = async (subplebbit) => {
  ...
  const signer = await plebbitPubsubApi.createSigner()
  const getRandomString = () => (Math.random() + 1).toString(36).replace('.', '')
  const comment = await plebbitPubsubApi.createComment({
    signer,
