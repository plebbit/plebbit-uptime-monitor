...
import {kuboPubsub, plebbitPubsubKuboApi, pubsubKuboApiUrl} from './plebbit-js/plebbit-js.js'
...
const pubsubSubscribeRetryForever = (pubsubTopic, onMessage, subplebbitAddress) => {
  const onError = async (error, fatal, pubsubMessage) => {
    debug(`kubo.pubsub.subscribe onError '${pubsubKuboApiUrl}' subplebbit '${subplebbitAddress}'`, {error, fatal, pubsubMessage})
    if (fatal) {
      try {
        await kuboPubsub.pubsub.unsubscribe(pubsubTopic, onMessage)
      }
      catch (e) {
        debug(`kubo.pubsub.unsubscribe error '${pubsubKuboApiUrl}' subplebbit '${subplebbitAddress}': ${e.message}`)
      }
      await trySubscribe()
    }
  }
...
const publishFakeChallengeRequest = async (subplebbit) => {
  ...
  const signer = await plebbitPubsubKuboApi.createSigner()
  const getRandomString = () => (Math.random() + 1).toString(36).replace('.', '')
  const comment = await plebbitPubsubKuboApi.createComment({
    signer,
