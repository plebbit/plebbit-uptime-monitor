...
const getPlebbitSeederMultiaddresses = async (plebbitSeederPeerId, subplebbitUpdateCid) => {
  ...
  if (!multiaddresses.length) {
    throw Error(`failed getting plebbit seeder '${plebbitSeederPeerId}' multiaddresses for cid '${subplebbitUpdateCid}': ${error?.message}`)
  }
...
const getSubplebbitUpdateCidFetchStats = async (plebbitSeederPeerId) => {
  ...
  while (attempts--) {
    try {
      ...
      const ma = multiaddr(websocketMultiaddress)
      ...
      let res, time
      try {
        res = await s.get(CID.parse(subplebbitUpdateCid))
        time = (Date.now() - before) / 1000
      }
      catch (e) {
        throw e
      }
        ...
        try {
          await libp2p.hangUp(ma.getPeerId())
        }
        catch (e) {
          debug(`error disconnecting from plebbit seeder '${websocketMultiaddress}': ${e.message}`)
        }
      }
      ...
      lastSubplebbitUpdateCidFetchTime = time
    }
    catch (e) {
      console.log(`${e.message}, retrying in ${retryDelay / 1000}s...`)
