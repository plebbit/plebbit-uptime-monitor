...
export const monitorPlebbitPreviewers = async () => {
  console.log(`monitoring ${config.monitoring.plebbitPreviewerUrls?.length} plebbit previewers: ${config.monitoring.plebbitPreviewerUrls?.join(' ')}`)
...
const getCommentPreviewFetchStats = async (plebbitPreviewerUrl) => {
  ...
  while (attempts--) {
    ...
    try {
      ...
      console.log(`fetched comment preview '${url}' in ${lastCommentPreviewFetchTime}s`)
      break
    }
    catch (e) {
      if (!attempts) {
        console.log(`failed fetching comment preview '${url}': ${e.message}`)
      }
    }
  }
...
// test
// console.log(await getCommentPreviewFetchStats('https://pleb.bz'))
