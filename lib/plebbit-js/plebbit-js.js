...
const pubsubKuboApiUrl = config.pubsubKuboApiUrl || config.kuboApiUrl
import {create as createKubo} from 'kubo-rpc-client'
import {Agent as HttpsAgent} from 'https'
import {Agent as HttpAgent} from 'http'
import Plebbit from '@plebbit/plebbit-js'

const plebbitKuboApi = await Plebbit({
  ...config.plebbitOptions,
  kuboRpcClientsOptions: [config.kuboApiUrl],
  pubsubKuboRpcClientsOptions: [config.kuboApiUrl]
})
plebbitKuboApi.on('error', error => {
  // console.log(error) // logging plebbit errors are only useful for debugging, not production
})
const plebbitPubsubKuboApi = await Plebbit({
  ...config.plebbitOptions,
  kuboRpcClientsOptions: [pubsubKuboApiUrl],
  pubsubKuboRpcClientsOptions: [pubsubKuboApiUrl]
})
plebbitPubsubKuboApi.on('error', error => {
  // console.log(error) // logging plebbit errors are only useful for debugging, not production
})
...
const Agent = config.kuboApiUrl?.startsWith('https') ? HttpsAgent : HttpAgent
