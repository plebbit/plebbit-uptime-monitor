...
export const monitorHttpRouters = async () => {
  console.log(`monitoring ${config.monitoring.httpRouterUrls?.length} http routers: ${config.monitoring.httpRouterUrls?.join(' ')}`)
    ...
    Providers: [
      {
        ...
        Payload: {
          ...
          Addrs: [
            ...
            `/ip${ipVersion}/${ip}/udp/4001/quic-v1/webtransport`,
          ]
        }
      }
    ]
...
const getProvidersFetchStats = async (httpRouterUrl) => {
  ...
  try {
    ...
    // TODO: check to make sure postProvidersRes.TTL exists
    lastPostProvidersFetchTime = (Date.now() - beforeTimestamp) / 1000
    console.log(`posted providers for cid '${fakeCid}' to '${httpRouterUrl}'`)

    // wait for http router to update
    await new Promise(r => setTimeout(r, waitForPostProvidersMs))

    console.log(`getting providers for cid '${fakeCid}' from '${httpRouterUrl}'`)
    ...
    console.log(`got providers for cid '${fakeCid}' from '${httpRouterUrl}' in ${lastGetProvidersFetchTime}s`)
  }
  catch (e) {
    console.log(`failed getting providers for cid '${fakeCid}' from '${httpRouterUrl}': ${e.message}`)
  }
...
// test
// console.log(await getProvidersFetchStats('https://peers.pleb.bot'))
// console.log(await getProvidersFetchStats('https://routing.lol'))
...
const getSubplebbitIpnsGetProvidersFetchStats = async (httpRouterUrl, subplebbitAddress, subplebbit) => {
  ...
  console.log(`getting providers for subplebbit '${subplebbit.address}' ipns '${suplebbitIpnsName}' from '${httpRouterUrl}'`)
  ...
  try {
    ...
    console.log(`got ${lastSubplebbitIpnsGetProvidersFetchProviderCount} providers for subplebbit '${subplebbit.address}' ipns '${suplebbitIpnsName}' from '${httpRouterUrl}' in ${lastSubplebbitIpnsGetProvidersFetchTime}s`)
  }
  catch (e) {
    console.log(`failed getting providers for subplebbit '${subplebbit.address}' ipns '${suplebbitIpnsName}' from '${httpRouterUrl}': ${e.message}`)
  }
...
// test
// console.log(await getSubplebbitIpnsGetProvidersFetchStats('https://peers.pleb.bot', 'plebtoken.eth', {address: 'plebtoken.eth', publicKey: 'oqb9NJrUccHpOHqfi1daakTAFup2BB7tYNbpkOcFOyE'}))
...
const prometheusObserveGetProvidersFetch = (httpRouterUrl, stats) => {
  ...
  // histograms
  if (isNumber(stats.lastGetProvidersFetchTime)) {
    histograms.getProvidersFetchDurationSeconds.observe(labels, stats.lastGetProvidersFetchTime)
  }
}
