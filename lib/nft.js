...
const debug = Debug('plebbit-uptime-monitor:nft')
...
export const monitorNfts = async () => {
  debug(`monitoring ${config.monitoring.nfts?.length} nfts: ${config.monitoring.nfts?.map(nft => nft.name).join(' ')}`)
...
const getIpfsGatewayFetchStats = async (nft, ipfsGatewayUrl) => {
  ...
  while (attempts--) {
    try {
      ...
      if (!attempts) {
        debug(`failed fetching nft '${nft.name}' metadata from ipfs gateway '${nftMetadataUrl}': ${e.message}`)
      }
    }
  }
  ...
  if (nftImageUrl) {
    ...
    while (attempts--) {
      try {
        ...
        debug(`fetched nft '${nft.name}' image from ipfs gateway '${nftImageUrl}' in ${lastIpfsGatewayFetchTime}s`)
        break
      }
      catch (e) {
        if (!attempts) {
          debug(`failed fetching nft '${nft.name}' image from ipfs gateway '${nftImageUrl}': ${e.message}`)
        }
      }
    }
  }
