...
const debug = Debug('plebbit-uptime-monitor:ipfs-gateway')
...
export const monitorIpfsGateways = async () => {
  debug(`monitoring ${config.monitoring.ipfsGatewayUrls?.length} ipfs gateways: ${config.monitoring.ipfsGatewayUrls?.join(' ')}`)
...
const fetchJsonRetry = async (fetchJsonRetryOptions) => {
  ...
  while (true) {
    ...
    try {
      ...
      debug(`${fetchJsonRetryOptions.retries - fetchJsonRetryOptions.attempts} retry left fetching '${fetchJsonRetryOptions.url}'`)
    }
  }
}
...
const getCommentFetchStats = async (ipfsGatewayUrl) => {
  ...
  debug(`adding comment to '${config.kuboApiUrl}' to monitor '${ipfsGatewayUrl}'...`)
  const {path: cid} = await pTimeout(kubo.add(JSON.stringify(fakeComment)), {
    milliseconds: 1000 * 60, 
    message: Error(`failed adding comment to '${config.kuboApiUrl}' (timed out) to monitor '${ipfsGatewayUrl}'`)
  })

  // wait for comment to propagate to ipfs
  debug(`added comment '${cid}' to ipfs to monitor '${ipfsGatewayUrl}', waiting ${waitForIpfsAddMs / 1000}s to propagate...`)
...
const getSubplebbitIpnsFetchStats = async (ipfsGatewayUrl, subplebbitAddress, subplebbit) => {
  ...
  debug(`fetching subplebbit '${subplebbit.address}' ipns '${suplebbitIpnsName}' from '${ipfsGatewayUrl}'`)
  ...
  try {
    ...
    debug(`fetched subplebbit '${subplebbit.address}' ipns '${suplebbitIpnsName}' from '${ipfsGatewayUrl}' in ${lastSubplebbitIpnsFetchTime}s`)
  }
  catch (e) {
    debug(`failed fetching subplebbit '${subplebbit.address}' ipns '${suplebbitIpnsName}' from '${ipfsGatewayUrl}': ${e.message}`)
  }
